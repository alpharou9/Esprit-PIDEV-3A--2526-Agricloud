...
class EventController extends AbstractController
{
    ...
    #[Route('/api/calendar-feed', name: 'event_calendar_feed', methods: ['GET'])]
    public function calendarFeed(Request $request, EventRepository $repo): JsonResponse
    {
        $start = $request->query->get('start');
        $end = $request->query->get('end');

        $rangeStart = $start
            ? (new \DateTimeImmutable((string) $start))->setTime(0, 0)
            : $this->resolveCalendarMonth((string) $request->query->get('month', ''));
        $rangeEnd = $end
            ? (new \DateTimeImmutable((string) $end))->setTime(0, 0)
            : $rangeStart->modify('first day of next month');

        $events = $repo->findBetweenDates($rangeStart, $rangeEnd);
    ...
    {
        $start = $event->getEventDate() instanceof \DateTimeInterface
            ? \DateTimeImmutable::createFromInterface($event->getEventDate())
            : new \DateTimeImmutable();
        $endSource = $event->getEndDate() instanceof \DateTimeInterface
            ? \DateTimeImmutable::createFromInterface($event->getEndDate())
            : $start->modify('+2 hours');
        $createdAt = $event->getCreatedAt() instanceof \DateTimeInterface
            ? \DateTimeImmutable::createFromInterface($event->getCreatedAt())
            : new \DateTimeImmutable();

        $content = implode("\r\n", [
            'BEGIN:VCALENDAR',
            'VERSION:2.0',
            'PRODID:-//AgriCloud//Events//EN',
            'BEGIN:VEVENT',
            'UID:event-' . ($event->getId() ?? uniqid('', true)) . '@agricloud',
            'DTSTAMP:' . $this->formatIcalDate($createdAt),
            'DTSTART:' . $this->formatIcalDate($start),
            'DTEND:' . $this->formatIcalDate($endSource),
            'SUMMARY:' . $this->escapeIcalText($event->getTitle()),
            'DESCRIPTION:' . $this->escapeIcalText($event->getDescription()),
            'LOCATION:' . $this->escapeIcalText($event->getLocation()),
            'URL:' . $this->generateUrl('event_show', ['id' => $event->getId()], UrlGeneratorInterface::ABSOLUTE_URL),
            'END:VEVENT',
            'END:VCALENDAR',
            '',
        ]);
        ...
        return new Response($content, Response::HTTP_OK, [
            ...
            'Content-Disposition' => sprintf('attachment; filename="event-%d.ics"', $event->getId() ?? 0),
        ]);
    }
    ...
    private function formatIcalDate(\DateTimeImmutable $date): string
    {
